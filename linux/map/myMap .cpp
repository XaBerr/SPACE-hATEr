...
bool myMap::downLoadTexture(float x, float y)
{
	...
	if(!mBotLeft.loadTextureFromFile(txt1.str()))
		if (!mBotLeft.loadTextureFromFile("map/default.jpg"))
			mLoad = false;

	if(!mTopLeft.loadTextureFromFile(txt2.str()))
		if (!mTopLeft.loadTextureFromFile("map/default.jpg"))
			mLoad = false;

	if(!mTopRight.loadTextureFromFile(txt3.str()))
		if (!mTopRight.loadTextureFromFile("map/default.jpg"))
			mLoad = false;

	if (!mBotRight.loadTextureFromFile(txt4.str()))
		if (!mBotRight.loadTextureFromFile("map/default.jpg"))
			mLoad = false;
